...
Raven Core 0.15.0 contains the following changes to the RPC interface and `carrot-cli` for multi-wallet:

* When running Raven Core with a single wallet, there are **no** changes to the RPC interface or `carrot-cli`. All RPC calls and `carrot-cli` commands continue to work as before.
...
- `uptime` returns the total runtime of the `carrotd` server since its last start (See [PR 10400](https://github.com/RavenProject/Ravencoin/pull/10400)).
