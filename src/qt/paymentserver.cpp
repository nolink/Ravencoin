...
const int CARROT_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString CARROT_IPC_PREFIX("carrot:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(CARROT_IPC_PREFIX, Qt::CaseInsensitive)) // carrot: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(CARROT_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(CARROT_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(CARROT_IPC_PREFIX, Qt::CaseInsensitive)) // carrot: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
